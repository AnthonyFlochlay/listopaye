...
public interface StartBeforeOrEqualPeriodTest {
    @TestFactory
    default Collection<DynamicTest> period_with_start_date_before_or_equal_other_period_starts_before_or_equal_that_period() {
        ...
        return somePeriodsStartingAfterOrEqual(thePeriodStartingBefore.startDateTime()).stream()
                .map(p -> dynamicTest("%s should starts before or equal %s".formatted(thePeriodStartingBefore, p), () -> assertThat(thePeriodStartingBefore.startsBeforeOrEqual(p)).isTrue()))
                .toList();
    }

    default List<Period> somePeriodsStartingAfterOrEqual(ZonedDateTime dateTime) {
        var dateTimePLus1Month = dateTime.plusMonths(1);
        return List.of(
                MonthlyPeriod.of(dateTimePLus1Month.getYear(), dateTimePLus1Month.getMonth()),
